...
### Troubleshooting container 404s (`/assets/...` or `manifest.json`)

If you see nginx errors like:

- `open() "/usr/share/nginx/html/assets/..." failed`
- `open() "/usr/share/nginx/html/manifest.json" failed`

then your reverse proxy is likely forwarding traffic to the container root (`/`) while the app is served under `/mas-admin/`.

Use the latest image and ensure your proxy preserves the `/mas-admin/` prefix.
